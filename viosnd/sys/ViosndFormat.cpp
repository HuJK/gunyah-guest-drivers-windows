...
static const ULONG ViosndRateTable[] = {
    ...
    _In_ ULONG Hz)
{
    ...
    Range->DataRange.SubFormat =
        Float ? KSDATAFORMAT_SUBTYPE_IEEE_FLOAT : KSDATAFORMAT_SUBTYPE_PCM;
    ...
    _Out_ PULONG RangeCount)
{
    ...
    *RangeCount = 0;
    ...
    PUCHAR block = (PUCHAR)ExAllocatePoolUninitialized(NonPagedPoolNx,
                                                       pointerBytes + rangeBytes,
                                                       VIOSND_POOL_TAG);
    if (block == NULL) {
    ...
    ULONG i = 0;
    if (Preferred != NULL) {
        ViosndFillRange(&ranges[i], Caps, Preferred->ContainerBits, Preferred->Float,
...
VOID
ViosndFreeDataRanges(
    _In_opt_ PKSDATARANGE *RangePointers)
{
    ...
    _Out_ PVIOSND_WAVE_FORMAT Out)
{
    ...
    if (HintRate != 0 && ViosndRateFromHz(HintRate, &chosenRate) &&
        (Caps->Rates & (1ULL << chosenRate)) != 0) {
        haveRate = TRUE;
    }
    if (!haveRate) {
        for (UCHAR r = 0; r < (UCHAR)SIZEOF_ARRAY(ViosndRateTable); ++r) {
            if ((Caps->Rates & (1ULL << r)) == 0 || ViosndRateTable[r] == 0 ||
                ViosndRateTable[r] > 48000) {
                continue;
            }
            chosenRate = r;
            haveRate = TRUE;
